import {instParseada} from './reconoceInst.js';

export type instGenerada = {
	enBinario: string,
	enHex: string
}

class GeneraInst {
	private enBinario: string;
	private enHex: string;
	...
	public generar(inst: instParseada): instGenerada {
		let instComentada: string;
	...
	private generarBinReg(num: string): string {
	...
	private generaBinConstAS(num: string): string {
	...
	private generaBinConstLS(num: string): string {
		let bin = Array.from(parseInt(num, 10).toString(2));
	...
	private generaBinConstCZ(num: string): string {
		let numero = parseInt(num, 10) > 0 ? parseInt(num, 10) : parseInt(num, 10) >>> 0;
		let bin = Array.from(numero.toString(2));
	...
	private generaBinConstB(num: string): string {
		let numero = parseInt(num, 10) > 0 ? parseInt(num, 10) : parseInt(num, 10) >>> 0;
		let bin = Array.from(numero.toString(2));
